...
- **MANDATORY**: Run `./gradlew build --dry-run` or `./gradlew build --dry-run` before applying any documentation generation
...
- **ENFORCEMENT**: Never proceed to Step 1 or any subsequent steps if `./gradlew build --dry-run` or `./gradlew build --dry-run` command fails or returns errors
...
Options:
- Yes, generate Javadoc HTML (./gradlew javadoc:javadoc)
- Yes, generate Javadoc and include in Maven Site (./gradlew clean site)
...
```bash
./gradlew spring-boot:run  # For Spring Boot projects
# or
...
- Validate that Javadoc generation works: `./gradlew javadoc:javadoc`
...
- **MUST** validate Javadoc generation with `./gradlew javadoc:javadoc`
- **MUST NOT** use generic or templated descriptions
...
- **MUST** run `./gradlew clean compile` to validate package-info.java files
- **MUST** run `./gradlew javadoc:javadoc` to validate Javadoc generation
...
- **MUST** run final `./gradlew clean verify` to ensure project builds successfully
...
- Verify changes with the command: `./gradlew compileJava` for package-info.java validation
- Verify changes with the command: `./gradlew javadoc:javadoc` for Javadoc validation
