...
- **MANDATORY**: Run `./gradlew build --dry-run` or `./gradlew build --dry-run` before applying any Maven best practices recommendations
...
- **BLOCKING SAFETY CHECK**: ALWAYS run `./gradlew compileJava` or `./gradlew compileJava` before ANY functional exception handling refactoring recommendations - compilation failure is a HARD STOP
- **CRITICAL VALIDATION**: Execute `./gradlew clean verify` or `./gradlew clean verify` to ensure all tests pass after applying functional exception handling patterns
