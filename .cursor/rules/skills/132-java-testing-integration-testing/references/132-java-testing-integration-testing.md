...
- **MANDATORY**: Run `./gradlew compileJava` or `./gradlew compileJava` before applying any change
...
- **DEPENDENCIES**: Requires that the project compiles successfully (run `./gradlew compileJava` first)
...
- **VALIDATE** that all generated code compiles by running `./gradlew test-compile` after the files are written

## Safeguards

- **BLOCKING SAFETY CHECK**: ALWAYS run `./gradlew compileJava` before generating integration-test scaffolding
...
- **CRITICAL VALIDATION**: Run `./gradlew test-compile` after writing generated files to confirm they compile
