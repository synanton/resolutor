...
- **BLOCKING SAFETY CHECK**: ALWAYS run `./gradlew compileJava` or `./gradlew compileJava` before ANY generics refactoring recommendations - compilation failure is a HARD STOP
- **CRITICAL VALIDATION**: Execute `./gradlew clean verify` or `./gradlew clean verify` to ensure all tests pass after applying generics improvements
